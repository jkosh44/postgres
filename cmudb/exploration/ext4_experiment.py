...
from data_copy import destroy_exploratory_data_cow, destroy_exploratory_data_cow_ext4, \
    copy_pgdata_cow_ext4
...
from util import PGDATA_LOC, EXPLORATION_PORT, \
    ...
    timed_execution, REPLICA, PRIMARY, execute_sys_command, UTF_8
...
def test_copy() -> Tuple[int, int, int, int, int, int, bool, str, int, int]:
    ...
    _, copy_time_ns = timed_execution(copy_pgdata_cow_ext4)
    ...
    _, snapshot_destroy_time = timed_execution(destroy_exploratory_data_cow_ext4)
...
def collect_results(benchbase_proc: subprocess.Popen, result_file: str):
    ...
    with open(result_file, "w") as f:
        ...
        while benchbase_proc.poll() is None:
            ...
            checkpoint_time_ns, copy_time_ns, docker_startup_time_ns, reset_wal_time_ns, postgres_startup_time_ns, \
            teardown_time_ns, valid, error_msg, precheckpoint_dirty_pages, postcheckpoint_dirty_pages = test_copy()
            if not first_obj:
                f.write(",\n")
            f.write("\t{\n")
...
def main():
    ...
    print("Clean any existing exploratory data")
    destroy_exploratory_data_cow_ext4()
    print("Existing exploratory data cleaned")
